...
> **バージョン**: v1.4（2026-05-06 更新）。過去の変更点は末尾「ログ」参照。
>
...
- **複数source 連結時のみ `__`（ダブルアンダースコア）を例外として使用**（v1.4 規定。utm_source 値内で複数リストを区切る）
...
### メルマガ（Benchmark）— **複数選択して 1URL に統合**（v1.4 で挙動変更）
...
→ ビルダーでメルマガを選ぶと**チェックボックス群**で複数選択可。**v1.3 期は配信先ごとに個別URLが生成されたが、v1.4 で 1URL 統合方式に変更**: 選択した複数リストの値を `__`（ダブルアンダースコア）で連結して utm_source の単一値に詰め込み、URL も Sheets ログも1本/1行に統合される。

例: 王道会員 + 王道1Day + 細菌検査 の3リスト同時配信
```
utm_source=email_benchmark_oudo_member__email_benchmark_oudo_1day__email_benchmark_kensa_users
```

GA4 の集計時は `utm_source` の **部分一致**（contains）で各リストごとの内訳が確認できる。
...
## ログ
- **2026-05-06 朝 — v1.4 改訂**: メルマガ複数リスト配信時の挙動を変更。
  - **旧（v1.3）**: 複数リスト選択時は配信先ごとに別URL生成・Sheetsに別行記録
  - **新（v1.4）**: 複数選択時も 1 URL に統合（`utm_source=A__B__C` の形）。Sheetsも1行記録
  - 理由: 実際のメルマガ送信は1つの本文に1つのURLを埋めて複数セグメントに一斉配信するのが自然。リストごとに別URLは運用上ニーズなし
  - GA4 では `utm_source` 部分一致フィルタ（contains）で各リスト内訳を確認可能
  - §1 基本原則に「`__` をsource複数連結用の例外として使用」と明記
  - §2 メルマガセクションを v1.4 仕様に書き換え（連結例つき）
